...
export const Filter = () => {
  ...
  const handleFilterWaist = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedWaist === value) {
      setSelectedWaist('');
      dispatch(filterCards({ waist: '', color: selectedColor, sleeve: selectedSleeve, price: selectedPrice }));
    } else {
        setSelectedWaist(value);
        dispatch(filterCards({ waist: value, color: selectedColor, sleeve: selectedSleeve, price: selectedPrice }));
      }    
    };
  ...
  const handleFilterColor = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedColor === value) {
      setSelectedColor('');
      dispatch(filterCards({ waist: selectedWaist, color: '', sleeve: selectedSleeve, price: selectedPrice }));
    } else {
        setSelectedColor(value);
        dispatch(filterCards({ waist: selectedWaist, color: value, sleeve: selectedSleeve, price: selectedPrice }));
      }
    };

  const handleFilterSleeve = (event: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = event.target;
    if (selectedSleeve === value) {
      setSelectedSleeve('');
      dispatch(filterCards({ waist: selectedWaist, color: selectedColor, sleeve: '', price: selectedPrice }));
    } else {
        setSelectedSleeve(value);
        dispatch(filterCards({ waist: selectedWaist, color: selectedColor, sleeve: value, price: selectedPrice }));
      }
    };
    ...
    const handleFilterPrice = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedPrice === value) {
          setSelectedPrice('');
          dispatch(filterCards({ waist: selectedWaist, color: selectedColor, sleeve: selectedSleeve, price: '' }));
        } else {
          setSelectedPrice(value);
          dispatch(filterCards({ waist: selectedWaist, color: selectedColor, sleeve: selectedSleeve, price: value }));
        }
      };
  ...
  return (
    <div className='container-filter'>
      ...
      <div className='container-filter-sleeve'>
        <h5>MANGA</h5>
        <div className='checklist-sleeve'>
            <input id='16' type='checkbox' onChange={handleFilterSleeve} value='MUSCULOSA' checked={selectedSleeve === 'MUSCULOSA'} />
            <label htmlFor='16'>MUSCULOSA</label>

            <input id='17' type='checkbox' onChange={handleFilterSleeve} value='CORTA' checked={selectedSleeve === 'CORTA'} />
            <label htmlFor='17'>CORTA</label>

            <input id='18' type='checkbox' onChange={handleFilterSleeve} value='3/4' checked={selectedSleeve === '3/4'} />
            <label htmlFor='18'>3/4</label>
        </div>
      </div>
